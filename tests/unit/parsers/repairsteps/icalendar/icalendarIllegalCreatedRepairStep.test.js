...
it('The repair step should inherit from AbstractRepairStep', () => {
	expect((new ICalendarIllegalCreatedRepairStep() instanceof AbstractRepairStep)).toEqual(true)
})

it('The repair step should have a priority', () => {
	expect(ICalendarIllegalCreatedRepairStep.priority()).toEqual(0)
})

it('The repair step should repair broken calendar data', () => {
	const repairStep = new ICalendarIllegalCreatedRepairStep()
	const brokenICS = getAsset('illegal-created')
	const fixedICS = getAsset('illegal-created-sanitized')
