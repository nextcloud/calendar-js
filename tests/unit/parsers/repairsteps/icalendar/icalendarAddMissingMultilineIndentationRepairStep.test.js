...
it('The repair step should inherit from AbstractRepairStep', () => {
	expect((new ICalendarAddMissingMultilineIndentationRepairStep() instanceof AbstractRepairStep)).toEqual(true)
})

it('The repair step should have a priority', () => {
	expect(ICalendarAddMissingMultilineIndentationRepairStep.priority()).toEqual(0)
})

it('The repair step should repair broken calendar data', () => {
	const repairStep = new ICalendarAddMissingMultilineIndentationRepairStep()
	const brokenICS = getAsset('invalid-line-folding')
	const fixedICS = getAsset('invalid-line-folding-sanitized')
