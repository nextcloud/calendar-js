...
it('The repair step should inherit from AbstractRepairStep', () => {
	expect((new ICalendarRemoveXNCGroupIdRepairStep() instanceof AbstractRepairStep)).toEqual(true)
})

it('The repair step should have a priority', () => {
	expect(ICalendarRemoveXNCGroupIdRepairStep.priority()).toEqual(0)
})

it('The repair step should repair broken calendar data', () => {
	const repairStep = new ICalendarRemoveXNCGroupIdRepairStep()
	const brokenICS = getAsset('x-nc-group-id')
	const fixedICS = getAsset('x-nc-group-id-sanitized')
